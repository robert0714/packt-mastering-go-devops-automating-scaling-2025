...
3. Configure local provider path: `terraform.rc` or `.terraformrc` 
   ...
   * content:
     ...
     ```hcl
      provider_installation {
        dev_overrides {
          "local/urlshortener"   = "C:/Users/robert0714/Desktop/tf-test/terraform.d/plugins"
          }
        direct {}
      }
     ```     
     In folder `tf-test` , create directory  `terraform.d/plugins/providers/local/urlshortener/0.0.1/windows_amd64`，並將編譯好的 provider binary 複製到此處。範例檔名：
     - `terraform-provider-urlshortener.exe`（Windows）
     - `terraform-provider-urlshortener`（Linux）
      ```bash
      terraform.d/
      └─ plugins/
        └─ providers/
          └─ local/
            └─ urlshortener/
              └─ 0.0.1/
                 └─ windows_amd64/
                   └─ terraform-provider-urlshortener.exe
      ```
4. cmd:
    ```bash
    ...
    or
    ```poweshell
    $env:TF_LOG = "TRACE";$env:TF_CLI_CONFIG_FILE = "$PWD\terraformrc"; terraform providers
    $env:TF_LOG = "TRACE";$env:TF_CLI_CONFIG_FILE = "$PWD\terraformrc"; terraform init -input=false
	# .terraform.lock.hcl is generated by terraform init
    $env:TF_LOG = "TRACE";$env:TF_CLI_CONFIG_FILE = "$PWD\terraformrc"; terraform apply -auto-approve
    ```  
